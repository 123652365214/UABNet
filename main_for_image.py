...
def parse_cfg():
    ...
    parser.add_argument("--config", type=str, default="/media/jiao/新加卷/wcm/wcm/configs/icod_train.py")
    parser.add_argument("--data-cfg", type=str, default="./dataset.yaml")
    parser.add_argument("--model-name", type=str, choices=model_zoo.__dict__.keys(), default='PvtV2B4_sarnet')
...
def main():
    ...
    if cfg.evaluate or cfg.has_test:
        io.save_weight(model=model, save_path=cfg.path.final_state_net)
        # io.load_weight(model=model,load_path='',strict=False)

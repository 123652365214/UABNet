...
def pytorch_map_step(att_weights, index, step, att_size, out_size):
    """替代 mobula.func.map_step：
    - att_weights: 注意力权重 [N, H, W]
    - index: 坐标索引 [N, H, W]
    - step: 步长（缩放因子）
    - att_size: 注意力图尺寸 (H_att, W_att)
    - out_size: 输出尺寸 (H_out, W_out)
    """
    N, H, W = att_weights.shape
    device = att_weights.device

    # 生成标准网格 [-1, 1]（grid_sample 的输入要求）
    y_coord = torch.linspace(-1, 1, H, device=device).view(1, H, 1).expand(N, H, W)
    x_coord = torch.linspace(-1, 1, W, device=device).view(1, 1, W).expand(N, H, W)

    # 添加偏移量（根据实际需求调整公式）
    grid_x = x_coord + att_weights * step  # X方向偏移
    grid_y = y_coord + index * step        # Y方向偏移

    return torch.stack([grid_x, grid_y], dim=-1)  # 返回 [N, H, W, 2]

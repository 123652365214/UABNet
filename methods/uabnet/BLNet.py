...
from methods.zoomnext.GatedConv import GatedConv2dWithActivation
...
class Local(nn.Module): # Local refinement module # Progressive convolution block
    def __init__(self, in_channel, out_channel):
        ...
        # self.dilate3_3 = nn.Sequential(
        #     nn.Conv2d(out_channel, out_channel, 3, padding=3, dilation=3), nn.BatchNorm2d(out_channel)
        # )
        #
        # self.dilate5_5 = nn.Sequential(
        #     nn.Conv2d(out_channel, out_channel, 3, padding=5, dilation=5), nn.BatchNorm2d(out_channel), nn.ReLU(True)
        # )
        #
        # self.dilate7_7 = nn.Sequential(
        #     nn.Conv2d(out_channel, out_channel, 3, padding=7, dilation=7), nn.BatchNorm2d(out_channel), nn.ReLU(True)
        # )

        self.DConv3 = nn.Sequential(
            nn.Conv2d(out_channel, out_channel, 3, padding=1, dilation=1,groups=out_channel),nn.BatchNorm2d(out_channel),nn.ReLU(True)
        )

        self.DConv5 = nn.Sequential(
            nn.Conv2d(out_channel, out_channel, 5, padding=2, dilation=1, groups=out_channel),nn.BatchNorm2d(out_channel),nn.ReLU(True)
        )

        self.DConv7 = nn.Sequential(
            nn.Conv2d(out_channel, out_channel, 7, padding=3, dilation=1, groups=out_channel),nn.BatchNorm2d(out_channel),nn.ReLU(True)
        )

        self.DConv3_3 = nn.Sequential(
            nn.Conv2d(out_channel, out_channel, 3, padding=1, dilation=1, groups=out_channel),
            nn.BatchNorm2d(out_channel), nn.ReLU(True)
        )

        self.DConv5_5 = nn.Sequential(
            nn.Conv2d(out_channel, out_channel, 5, padding=2, dilation=1, groups=out_channel),
            nn.BatchNorm2d(out_channel), nn.ReLU(True)
        )

        self.DConv7_7 = nn.Sequential(
    ...
    def forward(self, x):
        x0 = self.conv1(x)
        x1 = self.conv3(x0)
        x1_3 = self.dilate3(x1)  # 现在做的实验是dd
        ...
        x2_3 = self.DConv3_3(x_3)
        x2_5 = self.DConv5_5(x_3)
        x2_7 = self.DConv7_7(x_3)
        # x2_3 = self.dilate3(x_3)
        # x2_5 = self.dilate5(x_3)
        # x2_7 = self.dilate7(x_3)
